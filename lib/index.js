...
var classMembers = (function (superClass) {
  return function (_superClass) {
    ...
    _proto.parseClassAndInstanceVarDeclarators = function parseClassAndInstanceVarDeclarators(node, kind) {
      ...
      for (;;) {
        ...
        declarations.push(isStatic ? this.parseClassVariable(decl) : this.parseInstanceVariable(decl));
        if (!this.eat(types.comma)) break;
      }

      this.semicolon();
      return this.finishNode(node, isStatic ? "ClassVariableDeclaration" : "ClassInstanceVariableDeclaration");
    };

    _proto.parseClassOrInstanceVarHead = function parseClassOrInstanceVarHead(decl, isStatic) {
      decl.id = this.parseBindingAtom();
      this.checkLVal(decl.id, true, undefined, (isStatic ? "class variable" : "class instance variable") + " declaration");
    };

    _proto.parseInstanceVariable = function parseInstanceVariable(node) {
      if (!node.computed && !node.static && (node.id.name === "constructor" || node.id.value === "constructor")) {
          this.raise(node.id.start, "Classes may not have an instance variable named 'constructor'");
        }

      return this.finishNode(node, "ClassInstanceVariableDeclarator");
    };

    _proto.parseClassVariable = function parseClassVariable(node) {
      if (!node.computed && !node.static && (node.id.name === "constructor" || node.id.value === "constructor")) {
          this.raise(node.id.start, "Classes may not have a private class variable named 'constructor'");
        }

      return this.finishNode(node, "VariableDeclarator");
    };

    _proto.parseInstanceVariableName = function parseInstanceVariableName() {
    ...
    _proto.parseClassMember = function parseClassMember(classBody, member, state) {
      ...
      if (type === types._let || type === types._const) {
        var node = this.startNode();
        this.next();
        this.parseClassAndInstanceVarDeclarators(node, type);
        classBody.body.push(node);
        return;
      } else if (type === types._var) {
        this.unexpected();
      }

      return _superClass.prototype.parseClassMember.call(this, classBody, member, state);
    };

    _proto.parseSubscript = function parseSubscript(base, startPos, startLoc, noCalls, state) {
      if (this.eat(types.doubleColon)) {
...
var Tokenizer = function (_LocationParser) {
  ...
  _proto.readToken_plus_min = function readToken_plus_min(code) {
    ...
    if (this.hasPlugin("classMembers")) {
      if (code === 45 && next === 62) {
        this.state.pos += 1;
        this.finishToken(types.thinArrow);
        return;
      }
    }
  };
...
var mixinPluginNames = ["estree", "classMembers", "jsx", "flow", "typescript"];
var mixinPlugins = {
  estree: estree,
