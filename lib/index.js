...
var classMembers = (function (superClass) {
  return function (_superClass) {
      ...
      for (;;) {
        ...
        if (this.eat(types.eq)) {
          ...
          if (kind === types._const) {
            this.unexpected();
          } else if (decl.key && decl.key.type !== "Identifier" || decl.id && decl.id.type !== "Identifier") {
            this.unexpected();
          }
    ...
    _proto.parseInstanceVarHead = function parseInstanceVarHead(decl) {
      decl.key = this.parseBindingAtom();
      this.checkLVal(decl.key, true, undefined, "class instance variable declaration");
    };

    _proto.parseClassVarHead = function parseClassVarHead(decl) {
