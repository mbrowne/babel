...
const isKeyword = makePredicate("break case catch continue debugger default do else finally for function if return switch throw try var when while with null true false instanceof typeof void delete new in this let const class extends export import yield super");
...
class Tokenizer extends LocationParser {
  ...
  readToken_plus_min(code) {
    ...
    if (code === 45 && next === 62) {
      this.finishOp(types.thinArrow, 2);
      return;
    } else if (next === code) {
...
class StatementParser extends ExpressionParser {
  ...
  parseWhenClause() {
    ...
    if (this.match(types._if)) {
      this.next();
      node.matchGuard = this.parseParenExpression();
    }

    this.expect(types.thinArrow);
    node.body = this.parseStatement(true);
    return this.finishNode(node, "WhenClause");
  }

  parseWhenClausePattern() {
    let node;

    if (this.match(types.regexp)) {
      const value = this.state.value;
      node = this.parseLiteral(value.value, "RegExpLiteral");
      node.pattern = value.pattern;
      node.flags = value.flags;
  ...
  parseObjectMatchPattern() {
    ...
    while (true) {
      ...
      const node = this.startNode();
      node.key = this.match(types.num) || this.match(types.string) ? this.parseExprAtom() : this.parseIdentifier(true);
